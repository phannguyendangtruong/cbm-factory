...
public class UserServiceImpl implements UserService {
    ...
    @Autowired
    UserUpdateByAdminValidate userUpdateAdminValidate;
    ...
    public BasePaginationResponse getListWork(PageDTO page, HttpServletRequest request) {
        ...
        try {
            ...
            for (MaintenancePerson maintenancePerson2 : maintenancePerson) {
                MaintenanceRequest maintenanceRequest = maintenanceRepository
                        .findById(maintenancePerson2.getMaintenanceRequestId()).get();
                ListWorkDTO listWorkDTO = new ListWorkDTO(maintenanceRequest);
                list.add(listWorkDTO);
            }
            resp = new BasePaginationResponse(list, pageable.getPageNumber(), pageable.getPageSize(),
                    maintenancePerson.getTotalElements());
            log.info("Get list work by user id success");
            return resp;
        } catch (Exception e) {
            log.info("Get list work by user id error");
    ...
    public BasePaginationResponse getAll(Pageable pageable) {
        ...
        try {
            Page<UserReponseDTO> list = userRepository.findAllByFullNameDesc(pageable)
                    .map(user -> modelMapper.map(user, UserReponseDTO.class));
            BasePaginationResponse resp = new BasePaginationResponse(list.getContent(), pageable.getPageNumber(),
                    pageable.getPageSize(), list.getTotalElements());
            log.info("Get all user success");
            return resp;
        } catch (Exception e) {
            log.info("Get all user error");
            return new BasePaginationResponse(e.getMessage());
        }
    }
    ...
    public BaseResponse changeActive(UserActiveDTO userActiveDTO) {
        ...
        try {
            ...
            log.info("Change active success");
            return resp;
        } catch (Exception e) {
            log.info("Change active error");
            return new BaseResponse(e.getMessage());
        }
    }
    ...
    public BaseResponse detailWork(Long maintenanceRequestId, HttpServletRequest request) {
        ...
        List<MaintenancePerson> listPersorn = maintenancePersonRepository
                .findByMaintenanceRequestId(maintenanceRequestId);
        List<String> userSupport = new ArrayList<>();
        String userCreateName = userRepository
                .findById(maintenanceRepository.findById(maintenanceRequestId).get().getCreatedBy()).get()
                .getFullName();
        String userMainName = "";
        ...
        List<MaintenancePerson> maintenancePersons = maintenancePersonRepository
                .findByMaintenanceRequestId(maintenanceRequestId);
        List<ListPerson> listPersons = new ArrayList<>();
        for (MaintenancePerson maintenancePerson : maintenancePersons) {
            ListPerson listPerson = new ListPerson(maintenancePerson, maintenancePerson.getConfirm(),
                    maintenancePerson.getIsDone());
            listPersons.add(listPerson);
        }
        try {
            MaintenancePerson maintenancePerson = maintenancePersonRepository.findByMaintenanceRequestIdAndUserId(
                    maintenanceRequestId,
                    tokenStore.getSessionFromToken(RestUtils.getTokenFromHeader(request)).get().getCurrentUserId());
    ...
    public BaseResponse updateAccountByUser(UserUpdateDTO user, Errors validate, HttpServletRequest request) {
        log.info("Update account by user");
        try {
            BaseResponse resp = new BaseResponse();
            ...
            User userUpdate = userRepository.findById(
                    tokenStore.getSessionFromToken(RestUtils.getTokenFromHeader(request)).get().getCurrentUserId())
                    .orElse(null);
            if (userUpdate == null) {
                log.info("User not found");
            ...
            resp.setData(userReponseDTO);
            log.info("Update account by user success");
            return resp;
        } catch (Exception e) {
            log.info("Update account by user error");
            return new BaseResponse(e.getMessage());
        }
    }
    ...
    public BasePaginationResponse getListWorker(PageDTO page) {
        ...
        try {
            ...
            resp = new BasePaginationResponse(list, pageable.getPageNumber(), pageable.getPageSize(),
                    userList.getTotalElements());
            log.info("Get list worker success");
            return resp;
        } catch (Exception e) {
            log.info("Get list worker error");
            return new BasePaginationResponse(e.getMessage());
        }
    }
    ...
    public BaseResponse dashboardWorker(HttpServletRequest request) {
        ...
        try {
            ...
            log.info("Get dashboard worker success");
            return resp;
        } catch (Exception e) {
            log.info("Get dashboard worker error");
            return new BaseResponse(e.getMessage());
        }
    }
    ...
    public BaseResponse getEvaluateByWorker(HttpServletRequest request) {
        ...
        try {
            ...
            for (MaintenancePerson e : list) {
                Evaluate eva = evaluateRepository
                        .findByMaintenanceRequestIdAndIsDeletedFalse(e.getMaintenanceRequestId());
                if (eva != null) {
            ...
            log.info("Get evaluate by worker error");
            return new BaseResponse(e.getMessage());
        }
    }
